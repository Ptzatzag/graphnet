...
from graphnet.training.loss_functions import MSELoss, VonMisesFisher2DLoss, EuclideanDistanceLoss
...
def main(
    ...
    # Get predictions
    prediction_columns = [string + '_pred' for string in config["target"]]
    ...
    # Save models state dictionary
    model.save_state_dict(f"{result_path}/state_dict.pth")
    # Save model configuration as a YML file
    model.save_config(f"{result_path}/model.yml")
